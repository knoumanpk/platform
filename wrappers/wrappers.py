...
class PlatformFlattenedActionWrapper(gym.ActionWrapper):
    """
    Changes the format of the parameterised action space to conform to that of Platform-v0
    """
...
class ScaledParameterisedActionWrapper(gym.ActionWrapper):
    ...
    def action(self, action):
        """
        :purpose: rescale from [-1,1] to original action-parameter range.
        :param action: {tuple(2)}
        :return: action {tuple(2)}
        """
